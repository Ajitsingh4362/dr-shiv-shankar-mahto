...
export default function AdminFAQ() {
  ...
  return (
    <div className="admin-panel">
      ...
      {/* ADD/EDIT FORM */}
      {editing && (
        <div style={{ background: 'var(--ivory)', border: '1px solid rgba(15, 33, 56,0.1)', borderRadius: '4px', padding: '24px', marginBottom: '24px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {CATEGORIES.map(cat => (
                <button key={cat} onClick={() => setF('category', cat)} style={{ padding: '5px 14px', borderRadius: '100px', border: '1px solid', fontSize: '11px', fontFamily: 'var(--font-body)', fontWeight: 600, cursor: 'pointer', background: form.category === cat ? catColors[cat] : 'var(--white)', color: form.category === cat ? '#fff' : 'var(--text-muted)', borderColor: form.category === cat ? catColors[cat] : 'rgba(15, 33, 56,0.12)', transition: 'all 0.15s' }}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            <input value={form.question} onChange={e => setF('question', e.target.value)} placeholder="e.g. How long does treatment take?" style={{ width: '100%', padding: '10px 14px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', fontSize: '0.92rem', fontFamily: 'var(--font-body)', outline: 'none', fontWeight: 500 }} />
          </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <textarea value={form.answer} onChange={e => setF('answer', e.target.value)} placeholder="Write a clear, helpful answer..." rows={4} style={{ width: '100%', padding: '10px 14px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', fontSize: '0.88rem', fontFamily: 'var(--font-body)', outline: 'none', resize: 'vertical', lineHeight: 1.7 }} />
          </div>
      ...
      {loading ? <p className="admin-empty">Loading...</p> : filtered.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {filtered.map((faq, i) => (
            <div key={faq.id} style={{ background: 'var(--white)', border: `1px solid ${expandedId === faq.id ? 'rgba(185,145,79,0.3)' : 'rgba(15, 33, 56,0.08)'}`, borderRadius: '4px', overflow: 'hidden', opacity: faq.visible ? 1 : 0.6, transition: 'border-color 0.2s' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 16px', cursor: 'pointer' }} onClick={() => setExpandedId(expandedId === faq.id ? null : faq.id)}>
                ...
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexShrink: 0 }}>
                  <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '100px', background: catColors[faq.category] ? `${catColors[faq.category]}20` : 'rgba(15, 33, 56,0.06)', color: catColors[faq.category] || 'var(--navy-800)', fontFamily: 'var(--font-body)', fontWeight: 600 }}>{faq.category}</span>
                  {!faq.visible && <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '100px', background: 'rgba(15, 33, 56,0.06)', color: 'var(--text-muted)', fontFamily: 'var(--font-body)' }}>Hidden</span>}
              ...
              {/* Expanded answer */}
              {expandedId === faq.id && (
                <div style={{ padding: '0 16px 14px 32px', borderTop: '1px solid rgba(15, 33, 56,0.06)' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <button onClick={() => moveDown(i)} disabled={i === filtered.length - 1} style={{ width: '28px', height: '28px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', cursor: i === filtered.length - 1 ? 'default' : 'pointer', background: 'var(--white)', fontSize: '12px', opacity: i === filtered.length - 1 ? 0.3 : 1 }}>↓</button>
                    <button onClick={() => toggle(faq.id, !faq.visible)} style={{ padding: '5px 12px', fontSize: '11px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', cursor: 'pointer', background: 'var(--white)', fontFamily: 'var(--font-body)', color: 'var(--text-muted)' }}>{faq.visible ? '👁 Hide' : '👁 Show'}</button>
