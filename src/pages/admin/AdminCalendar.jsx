  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(7,15,28,0.55)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }} onClick={onClose}>
      <div style={{ background: 'var(--white)', borderRadius: '2px', padding: '0', maxWidth: '440px', width: '100%', boxShadow: '0 20px 60px rgba(7,15,28,0.2)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
        ...
        {/* Actions */}
        <div style={{ padding: '14px 24px 20px', display: 'flex', gap: '8px', flexWrap: 'wrap', borderTop: '1px solid rgba(15, 33, 56,0.08)' }}>
...
export default function AdminCalendar() {
  ...
  async function updateStatus(id, status) {
    ...
    if (status === 'confirmed') {
      ...
      if (appt) {
        const phone = (appt.phone || '').replace(/[^\d]/g, '')
        const msg = encodeURIComponent(`Hi ${appt.name}, your appointment with Dr. Shiv Shankar Mahto has been confirmed${appt.preferred_date ? ` for ${new Date(appt.preferred_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'long' })}` : ''}${appt.preferred_time ? ` at ${appt.preferred_time}` : ''}. Looking forward to seeing you! 🌿${WHATSAPP_FOOTER}`)
  ...
  return (
    <div className="admin-panel" style={{ maxWidth: '1100px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setCurrent(new Date())} style={{ padding: '7px 14px', fontSize: '11px', fontFamily: 'var(--font-body)', fontWeight: 600, border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', cursor: 'pointer', background: 'var(--white)', color: 'var(--text-muted)' }}>Today</button>

        <div style={{ display: 'flex', gap: '4px' }}>
          <button onClick={prevPeriod} style={{ width: '32px', height: '32px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', cursor: 'pointer', background: 'var(--white)', fontSize: '14px' }}>‹</button>
          <button onClick={nextPeriod} style={{ width: '32px', height: '32px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', cursor: 'pointer', background: 'var(--white)', fontSize: '14px' }}>›</button>
        </div>

        <div style={{ display: 'flex', gap: '4px' }}>
          {['month', 'week'].map(v => (
            <button key={v} onClick={() => setView(v)} style={{ padding: '7px 16px', fontSize: '11px', fontFamily: 'var(--font-body)', fontWeight: 600, border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', cursor: 'pointer', background: view === v ? 'var(--navy-800)' : 'var(--white)', color: view === v ? 'var(--gold-pale)' : 'var(--text-muted)', textTransform: 'capitalize' }}>
              {v}
            </button>
          ))}
        </div>
      ...
      {loading ? <p className="admin-empty">Loading...</p> : (
        <>
          {/* ===== MONTH VIEW ===== */}
          {view === 'month' && (
            <div className="admin-cal-scroll" style={{ background: 'var(--white)', border: '1px solid rgba(15, 33, 56,0.08)', borderRadius: '2px', overflow: 'auto' }}>
              <div className="admin-cal-month-inner" style={{ minWidth: '640px' }}>
                {/* Day headers */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid rgba(15, 33, 56,0.08)' }}>
                  {DAYS.map(d => (
                    <div key={d} style={{ padding: '10px 8px', textAlign: 'center', fontSize: '10px', fontFamily: 'var(--font-body)', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', color: 'var(--text-muted)', borderRight: '1px solid rgba(15, 33, 56,0.06)' }}>{d}</div>
                  ))}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
                  {getMonthDays().map((date, i) => {
                    ...
                    return (
                      <div key={i} className="admin-cal-day-cell" style={{ minHeight: '100px', padding: '8px', borderRight: '1px solid rgba(15, 33, 56,0.06)', borderBottom: '1px solid rgba(15, 33, 56,0.06)', background: isOtherMonth ? 'rgba(15, 33, 56,0.02)' : 'var(--white)', transition: 'background 0.15s' }}
                        onMouseEnter={e => { if (!isOtherMonth) e.currentTarget.style.background = 'rgba(13, 148, 136,0.03)' }}
          ...
          {/* ===== WEEK VIEW ===== */}
          {view === 'week' && (
            <div style={{ background: 'var(--white)', border: '1px solid rgba(15, 33, 56,0.08)', borderRadius: '2px', overflow: 'auto' }}>
              <div style={{ minWidth: '600px' }}>
                {/* Day headers */}
                <div style={{ display: 'grid', gridTemplateColumns: '80px repeat(7, 1fr)', borderBottom: '2px solid rgba(15, 33, 56,0.08)' }}>
                  <div style={{ padding: '12px 8px' }} />
                  {weekDays.map((d, i) => {
                    const isToday = isSameDay(d, today)
                    return (
                      <div key={i} style={{ padding: '12px 8px', textAlign: 'center', borderLeft: '1px solid rgba(15, 33, 56,0.06)', background: isToday ? 'rgba(13, 148, 136,0.05)' : 'transparent' }}>
                ...
                {/* Time slots */}
                {TIME_SLOTS.map(slot => (
                  <div key={slot} style={{ display: 'grid', gridTemplateColumns: '80px repeat(7, 1fr)', borderBottom: '1px solid rgba(15, 33, 56,0.06)', minHeight: '64px' }}>
                    ...
                    {weekDays.map((d, i) => {
                      ...
                      const isToday = isSameDay(d, today)
                      return (
                        <div key={i} style={{ padding: '4px 6px', borderLeft: '1px solid rgba(15, 33, 56,0.06)', background: isToday ? 'rgba(13, 148, 136,0.03)' : 'transparent', minHeight: '64px' }}>
