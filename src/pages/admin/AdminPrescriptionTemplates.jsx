...
function MedicineRow({ med, idx, onChange, onDelete, isOnly }) {
  ...
  return (
    <div style={{ background: 'var(--white)', border: '1px solid rgba(15, 33, 56,0.1)', borderRadius: '4px', padding: '14px 16px', position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
        ...
        <input value={med.name} onChange={e => s('name', e.target.value)} placeholder="Medicine name (e.g. Pulsatilla)" style={{ flex: 1, padding: '8px 12px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', fontSize: '0.92rem', fontFamily: 'var(--font-body)', fontWeight: 600, outline: 'none', color: 'var(--navy-800)' }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: '8px' }}>
        <div>
          ...
          <select value={med.dosage} onChange={e => s('dosage', e.target.value)} style={{ width: '100%', padding: '7px 8px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', fontSize: '0.8rem', fontFamily: 'var(--font-body)', outline: 'none' }}>
        ...
        <div>
          ...
          <select value={med.frequency} onChange={e => s('frequency', e.target.value)} style={{ width: '100%', padding: '7px 8px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', fontSize: '0.8rem', fontFamily: 'var(--font-body)', outline: 'none' }}>
        ...
        <div>
          ...
          <select value={med.timing} onChange={e => s('timing', e.target.value)} style={{ width: '100%', padding: '7px 8px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', fontSize: '0.8rem', fontFamily: 'var(--font-body)', outline: 'none' }}>
        ...
        <div>
          ...
          <input value={med.duration} onChange={e => s('duration', e.target.value)} placeholder="e.g. 5 days" style={{ width: '100%', padding: '7px 8px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', fontSize: '0.8rem', fontFamily: 'var(--font-body)', outline: 'none' }} />
        </div>
      </div>
      <div style={{ marginTop: '8px' }}>
        <input value={med.notes} onChange={e => s('notes', e.target.value)} placeholder="Special notes for this medicine (optional)" style={{ width: '100%', padding: '7px 10px', border: '1px solid rgba(15, 33, 56,0.08)', borderRadius: '2px', fontSize: '0.8rem', fontFamily: 'var(--font-body)', outline: 'none', color: 'var(--text-muted)' }} />
      </div>
    </div>
  )
}
    ...
    <div className="admin-panel" style={{ maxWidth: '900px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          {COMMON_TAGS.map(tag => (
            <button key={tag} onClick={() => toggleTag(tag)} style={{ padding: '5px 12px', borderRadius: '100px', border: '1px solid', fontSize: '11px', fontFamily: 'var(--font-body)', fontWeight: 500, cursor: 'pointer', background: (form.condition_tags || []).includes(tag) ? 'var(--navy-800)' : 'var(--white)', color: (form.condition_tags || []).includes(tag) ? 'var(--gold-pale)' : 'var(--text-muted)', borderColor: (form.condition_tags || []).includes(tag) ? 'var(--navy-800)' : 'rgba(15, 33, 56,0.12)', transition: 'all 0.15s' }}>
              {tag}
            </button>
          ))}
        </div>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {FOLLOW_UPS.map(f => (
            <button key={f} onClick={() => setF('follow_up_duration', f)} style={{ padding: '7px 16px', borderRadius: '2px', border: '1px solid', fontSize: '12px', fontFamily: 'var(--font-body)', fontWeight: 500, cursor: 'pointer', background: form.follow_up_duration === f ? 'var(--navy-800)' : 'var(--white)', color: form.follow_up_duration === f ? 'var(--gold-pale)' : 'var(--text-muted)', borderColor: form.follow_up_duration === f ? 'var(--navy-800)' : 'rgba(15, 33, 56,0.12)', transition: 'all 0.15s' }}>
              {f}
            </button>
          ))}
        </div>
    ...
    <div className="admin-panel">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '12px', marginBottom: '24px' }}>
        {[
          ...
          <div key={i} style={{ background: 'var(--white)', border: '1px solid rgba(15, 33, 56,0.08)', borderRadius: '2px', padding: '14px 16px' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <input placeholder="Search templates or conditions..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, minWidth: '200px', padding: '9px 14px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', fontSize: '0.88rem', fontFamily: 'var(--font-body)', outline: 'none' }} />
        <select value={filterCat} onChange={e => setFilterCat(e.target.value)} style={{ padding: '9px 14px', border: '1px solid rgba(15, 33, 56,0.12)', borderRadius: '2px', fontSize: '0.88rem', fontFamily: 'var(--font-body)', outline: 'none' }}>
      ...
      {loading ? <p className="admin-empty">Loading...</p> : filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
          {filtered.map(t => {
            ...
            return (
              <div key={t.id} style={{ background: 'var(--white)', border: '1px solid rgba(15, 33, 56,0.08)', borderRadius: '4px', overflow: 'hidden', transition: 'box-shadow 0.2s' }}
                onMouseEnter={e => e.currentTarget.style.boxShadow = '0 4px 20px rgba(15, 33, 56,0.08)'}
                ...
                {(t.condition_tags || []).length > 0 && (
                  <div style={{ padding: '10px 16px 6px', display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {(t.condition_tags || []).map(tag => (
                      <span key={tag} style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '100px', background: 'rgba(15, 33, 56,0.06)', color: 'var(--navy-800)', fontFamily: 'var(--font-body)' }}>{tag}</span>
                    ))}
                  </div>
                )}
                ...
                {/* Actions */}
                <div style={{ padding: '10px 16px 14px', borderTop: '1px solid rgba(15, 33, 56,0.06)', display: 'flex', gap: '6px' }}>
